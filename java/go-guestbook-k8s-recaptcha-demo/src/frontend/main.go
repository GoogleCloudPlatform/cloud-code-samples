...
var recaptchaSecret = "6Lc7__MUAAAAAKyVRlkh58HNCy91lpCZ2FBWdjyL"
...
func (f *frontendServer) postHandler(w http.ResponseWriter, r *http.Request) {
	...
	var recaptchaToken = r.FormValue("g-recaptcha-response");

	if recaptchaToken == "" {
		http.Error(w, "unable to verify your humanity", http.StatusForbidden)
		return
	}
	recaptchaResponse, err := http.PostForm("https://www.google.com/recaptcha/api/siteverify",  url.Values{
		"secret": {recaptchaSecret},
		"response": {recaptchaToken},
		"remoteip": {getIP(r)}})

	recaptchaResult := new(RecaptchaResponse)

	if (err == nil) {
		json.NewDecoder(recaptchaResponse.Body).Decode(recaptchaResult)
		if (!recaptchaResult.Success) {
			http.Error(w, "unable to verify your humanity", http.StatusForbidden)
			return
		} else {
			log.Print("Humanity confirmed!!")
		}

	} else {
		log.Print("Recaptcha backend having trouble. Failing open")
	}
